...
#define JSON_FLAG(_NAME, _VALUE)                                 \
  str += String('"') + String(FPSTR(_NAME)) + String(F("\":")) + \
         String(_VALUE ? "true" : "false") + String(',');

#define JSON_VALUE(_NAME, _VALUE)                                  \
  str += String('"') + String(FPSTR(_NAME)) + String(F("\":\"")) + \
         String(_VALUE) + String(F("\","));
...
void get_json(String& str) {
  ...
  // BAT
  JSON_FLAG(S_JSON_BAT_PRE, data.bat.pre);
...
const settings_t& getAllSettings() { return data; }

const version_t& getVersion() { return data.version; }

const autosave_settings_t& getAutosaveSettings() { return data.autosave; }

const attack_settings_t& getAttackSettings() { return data.attack; }

const wifi_settings_t& getWifiSettings() { return data.wifi; }

const bat_settings_t& getBATSettings() { return data.bat; }

const sniffer_settings_t& getSnifferSettings() { return data.sniffer; }

const access_point_settings_t& getAccessPointSettings() { return data.ap; }

const web_settings_t& getWebSettings() { return data.web; }

const cli_settings_t& getCLISettings() { return data.cli; }

const led_settings_t& getLEDSettings() { return data.led; }

const display_settings_t& getDisplaySettings() { return data.display; }
...
void setBATSettings(const bat_settings_t& bat) {
  data.bat = bat;
  changed = true;
}
