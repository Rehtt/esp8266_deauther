...
void CLI::runCommand(String input) {
  ...
  // load [<type>] [<file>]
    ...
    if (list->size() == 3) {  // Todo: check if -f or filename
      ...
      // else if (eqlsCMD(1, CLI_SETTING)) load ? settings::load(list->get(2)) :
      // settings::save(true, list->get(2));
      else
        parameterError(list->get(1));
    } else {
      if (eqlsCMD(1, CLI_SSID))
        load ? ssids.load() : ssids.save(true);
      else if (eqlsCMD(1, CLI_NAME))
        load ? names.load() : names.save(true);
      else if (eqlsCMD(1, CLI_SETTING))
        load ? settings::load() : settings::save(true);
  ...
  // get <setting>
    ...
    // Version
    else if (eqls(str, S_JSON_VERSION))
      prntln(DEAUTHER_VERSION);
    else if (eqls(str, S_JSON_AUTOSAVE))
      prntln(settings::getAutosaveSettings().enabled);
    else if (eqls(str, S_JSON_AUTOSAVETIME))
      prntln(settings::getAutosaveSettings().time);

    // Attack
    else if (eqls(str, S_JSON_BEACONCHANNEL))
      prntln((int)settings::getAttackSettings().attack_all_ch);
    else if (eqls(str, S_JSON_RANDOMTX))
      prntln(settings::getAttackSettings().random_tx);
    else if (eqls(str, S_JSON_ATTACKTIMEOUT))
      prntln(settings::getAttackSettings().timeout);
    else if (eqls(str, S_JSON_DEAUTHSPERTARGET))
      prntln(settings::getAttackSettings().deauths_per_target);
    else if (eqls(str, S_JSON_DEAUTHREASON))
      prntln(settings::getAttackSettings().deauth_reason);
    else if (eqls(str, S_JSON_BEACONINTERVAL))
      prntln((bool)settings::getAttackSettings().beacon_interval);
    else if (eqls(str, S_JSON_PROBESPERSSID))
      prntln(settings::getAttackSettings().probe_frames_per_ssid);

    // WiFi
    else if (eqls(str, S_JSON_CHANNEL))
      prntln(settings::getWifiSettings().channel);
    else if (eqls(str, S_JSON_MACST))
      prntln(macToStr(settings::getWifiSettings().mac_st));
    else if (eqls(str, S_JSON_MACAP))
      prntln(macToStr(settings::getWifiSettings().mac_ap));

    // Sniffer
    else if (eqls(str, S_JSON_CHTIME))
      prntln(settings::getSnifferSettings().channel_time);
    else if (eqls(str, S_JSON_MIN_DEAUTHS))
      prntln(settings::getSnifferSettings().min_deauth_frames);

    // AP
    else if (eqls(str, S_JSON_SSID))
      prntln(settings::getAccessPointSettings().ssid);
    else if (eqls(str, S_JSON_PASSWORD))
      prntln(settings::getAccessPointSettings().password);
    else if (eqls(str, S_JSON_HIDDEN))
      prntln(settings::getAccessPointSettings().hidden);
    else if (eqls(str, S_JSON_IP))
      prntln(settings::getAccessPointSettings().ip);

    // Web
    else if (eqls(str, S_JSON_WEBINTERFACE))
      prntln(settings::getWebSettings().enabled);
    else if (eqls(str, S_JSON_CAPTIVEPORTAL))
      prntln(settings::getWebSettings().captive_portal);
    else if (eqls(str, S_JSON_WEB_SPIFFS))
      prntln(settings::getWebSettings().use_spiffs);
    else if (eqls(str, S_JSON_LANG))
      prntln(settings::getWebSettings().lang, 3);

    // CLI
    else if (eqls(str, S_JSON_SERIALINTERFACE))
      prntln(settings::getCLISettings().enabled);
    else if (eqls(str, S_JSON_SERIAL_ECHO))
      prntln(settings::getCLISettings().serial_echo);

    // LED
    else if (eqls(str, S_JSON_LEDENABLED))
      prntln(settings::getLEDSettings().enabled);

    // Display
    else if (eqls(str, S_JSON_DISPLAYINTERFACE))
      prntln(settings::getDisplaySettings().enabled);
    else if (eqls(str, S_JSON_DISPLAY_TIMEOUT))
      prntln(settings::getDisplaySettings().timeout);
  ...
  // set <setting> <value>
    ...
    // Autosave
    if (eqls(str, S_JSON_AUTOSAVE))
      newSettings.autosave.enabled = boolVal;
    else if (eqls(str, S_JSON_AUTOSAVETIME))
      newSettings.autosave.time = unsignedVal;

    // Attack
    else if (eqls(str, S_JSON_BEACONCHANNEL))
      newSettings.attack.attack_all_ch = boolVal;
    else if (eqls(str, S_JSON_RANDOMTX))
      newSettings.attack.random_tx = boolVal;
    else if (eqls(str, S_JSON_ATTACKTIMEOUT))
      newSettings.attack.timeout = unsignedVal;
    else if (eqls(str, S_JSON_DEAUTHSPERTARGET))
      newSettings.attack.deauths_per_target = unsignedVal;
    else if (eqls(str, S_JSON_DEAUTHREASON))
      newSettings.attack.deauth_reason = unsignedVal;
    else if (eqls(str, S_JSON_BEACONINTERVAL))
      newSettings.attack.beacon_interval = (beacon_interval_t)boolVal;
    else if (eqls(str, S_JSON_PROBESPERSSID))
      newSettings.attack.probe_frames_per_ssid = unsignedVal;

    // WiFi
    else if (eqls(str, S_JSON_CHANNEL))
      newSettings.wifi.channel = unsignedVal;
    else if (eqls(str, S_JSON_MACST))
      strToMac(strVal, newSettings.wifi.mac_st);
    else if (eqls(str, S_JSON_MACAP))
      strToMac(strVal, newSettings.wifi.mac_ap);

    // Sniffer
    else if (eqls(str, S_JSON_CHTIME))
      newSettings.sniffer.channel_time = unsignedVal;
    else if (eqls(str, S_JSON_MIN_DEAUTHS))
      newSettings.sniffer.min_deauth_frames = unsignedVal;

    // AP
    else if (eqls(str, S_JSON_SSID))
      strncpy(newSettings.ap.ssid, strVal.c_str(), 32);
    else if (eqls(str, S_JSON_PASSWORD))
      strncpy(newSettings.ap.password, strVal.c_str(), 64);
    else if (eqls(str, S_JSON_HIDDEN))
      newSettings.ap.hidden = boolVal;
    else if (eqls(str, S_JSON_IP))
      strToIP(strVal, newSettings.ap.ip);

    else if (eqls(str, S_JSON_BAT_PRE))
      newSettings.bat.pre = boolVal;
    // Web
    else if (eqls(str, S_JSON_WEBINTERFACE))
      newSettings.web.enabled = boolVal;
    else if (eqls(str, S_JSON_CAPTIVEPORTAL))
      newSettings.web.captive_portal = boolVal;
    else if (eqls(str, S_JSON_WEB_SPIFFS))
      newSettings.web.use_spiffs = boolVal;
    else if (eqls(str, S_JSON_LANG))
      strncpy(newSettings.web.lang, strVal.c_str(), 3);

    // CLI
    else if (eqls(str, S_JSON_SERIALINTERFACE))
      newSettings.cli.enabled = boolVal;
    else if (eqls(str, S_JSON_SERIAL_ECHO))
      newSettings.cli.serial_echo = boolVal;

    // LED
    else if (eqls(str, S_JSON_LEDENABLED))
      newSettings.led.enabled = boolVal;

    // Display
    else if (eqls(str, S_JSON_DISPLAYINTERFACE))
      newSettings.display.enabled = boolVal;
    else if (eqls(str, S_JSON_DISPLAY_TIMEOUT))
      newSettings.display.timeout = unsignedVal;
  ...
  // sysinfo
    ...
    sprintf(
        s, str(CLI_SYSTEM_RAM_OUT).c_str(), fs_info.usedBytes,
        fs_info.usedBytes / (fs_info.totalBytes / 100),
        fs_info.totalBytes - fs_info.usedBytes,
        (fs_info.totalBytes - fs_info.usedBytes) / (fs_info.totalBytes / 100),
        fs_info.totalBytes);
    prnt(String(s));
  ...
  // led <r> <g> <b> [<brightness>]
    ...
    led::setColor(list->get(1).toInt(), list->get(2).toInt(),
                  list->get(3).toInt());
  }
  ...
  // screen mode <menu/packetmonitor/buttontest/loading>
    ...
    if (eqlsCMD(2, CLI_MODE_BUTTONTEST))
      displayUI.mode = DISPLAY_MODE::BUTTON_TEST;
    else if (eqlsCMD(2, CLI_MODE_PACKETMONITOR))
      displayUI.mode = DISPLAY_MODE::PACKETMONITOR;
    else if (eqlsCMD(2, CLI_MODE_LOADINGSCREEN))
      displayUI.mode = DISPLAY_MODE::LOADSCAN;
